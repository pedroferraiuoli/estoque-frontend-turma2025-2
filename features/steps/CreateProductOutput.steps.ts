...
Given('que criei um produto, uma entrada e um pedido para saida', { timeout: 30000 }, async function (this: CustomWorld) {
  // Criar produto
...
When('navego para a tela de nova saida', async function (this: CustomWorld) {
...
When('preencho os dados da saida com o barcode do produto e a quantidade deve ser menor ou igual que o estoque', async function (this: CustomWorld) {
  await this.page!.getByRole('textbox', { name: 'Código de Barras do Produto' }).fill(barcode.toString());
  await this.page!.getByRole('spinbutton', { name: 'Quantidade' }).fill('85');
  await this.page!.locator('#outputDate').fill('2025-07-15T10:00');
})

When('solicito a criação da saida', async function (this: CustomWorld) {
  await this.page!.getByRole('button', { name: 'Criar Saída' }).click();
  await this.page!.waitForTimeout(2000);
})

Then('devo ver os detalhes da saida criada', { timeout: 15000 }, async function (this: CustomWorld) {
  await expect(this.page!.getByText('Saída criada com sucesso!')).toBeVisible();
  await expect(this.page!.getByTestId('output-quantity')).toHaveText('85');
})

Then('o estoque do produto deve estar atualizado na saida', { timeout: 15000 }, async function (this: CustomWorld) {
  await this.page!.getByRole('link', { name: 'Produtos', exact: true }).click();
  await this.page!.waitForTimeout(2000);
  ...
  await expect(this.page!.getByTestId('product-quantity')).toHaveText('15');
})
